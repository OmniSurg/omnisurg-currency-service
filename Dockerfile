# Deploy image for omnisurg-currency-service. Built by the CI image job
# (reusable-go-service-ci.yml) from the SINGLE-repo context and pushed to
# ghcr.io/omnisurg/omnisurg-currency-service. The runtime mirrors the local
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD wget -qO- http://localhost:8092/api/v1/currency/health || exit 1
